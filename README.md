...
The test fixture (`fixtures/test.ts`, H.264+Opus, 10 s loop) is generated by
`./fixtures/make-fixture.sh` (gitignored — media isn't committed; run it after a
fresh clone, the smoke test needs it). `fixtures/stream.sh` is a live ffmpeg
...
### CLI, certificates, production

Full CLI reference, certificate-mode details (self-signed vs mkcert), and
production deployment (supervisord) live in
[`docs/reference-gateway.md`](docs/reference-gateway.md).
...
WebSRT/
  ...
  docs/
    ...
    reference-gateway.md      # reference gateway: CLI reference, cert modes, supervisord deployment
  fixtures/
    ...
    make-fixture.sh           # generates test.ts (gitignored; 10 s H.264+Opus loop)
  certs/
