...
// `sends` counts SRT SendPacket actions (ACK/NAK/KeepAlive/...) generated by
// the protocol per interval. Compared against the main thread's `written`
// count and the gateway's `rx_ack`, this pinpoints where outgoing control
// traffic is lost (worker→main postMessage, main→WT write, or WT→gateway).
let stats = { batches: 0, datagrams: 0, maxBatchMs: 0, pollMs: 0, pollCount: 0, sends: 0 };
...
async function doInit(latencyMs: number) {
  ...
  statsTimer = setInterval(() => {
    ...
    if (stats.batches > 0 || stats.pollCount > 0) {
      queue({ type: 'log', msg: `worker: ${stats.batches} batches, ${stats.datagrams} dgrams, sends=${stats.sends}, maxBatch=${stats.maxBatchMs.toFixed(1)}ms, poll=${stats.pollCount}× maxPoll=${stats.pollMs.toFixed(1)}ms`, cls: 'info' });
    }
    stats = { batches: 0, datagrams: 0, maxBatchMs: 0, pollMs: 0, pollCount: 0, sends: 0 };
    flushOutgoing();
  }, 5000);
}
