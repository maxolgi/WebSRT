    ...
    match header.bits_per_sample {
        ...
        16 => {
            for chunk in data.chunks_exact(5) {
                let s1_u16: u16 =
                    ((reverse_byte(chunk[1]) as u16) << 8) | (reverse_byte(chunk[0]) as u16);
...
mod tests {
    ...
    fn test_reverse_byte_self_inverse() {
        for b in [0x00u8, 0x01, 0x12, 0x55, 0x7F, 0x80, 0xAA, 0xFF, 0x39, 0xDE] {
            assert_eq!(
                reverse_byte(reverse_byte(b)),
                b,
                "not self-inverse for {b:#04x}"
            );
        }
    }
    ...
    fn test_unwrap_bad_header_returns_empty() {
        ...
        assert!(
            out.is_empty(),
            "invalid bits_per_sample should yield no samples"
        );
    }
