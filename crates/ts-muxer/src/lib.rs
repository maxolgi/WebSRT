//! `ts-muxer` — platform-native MPEG-TS muxer.
//!
//! Takes H.264/HEVC/AV1 NAL units (Annex B) and audio packets and produces
//! 188-byte ISO/IEC 13818-1 MPEG-TS packets. Callers drive it by pushing
//! encoded chunks via `push_video` / `push_audio` / `push_pcm` and draining
//! finished packets via `poll`. The browser binding lives in `ts-muxer-wasm`.
...
impl TsMuxer {
    ...
    /// (registered via `add_audio_pid`). Unknown pids and non-s302m streams
    /// are a no-op.
